...
{
    public class MatchListPipeline
    {
        ...
        // Proste komendy sterujące rozmową (user wpisuje w rtb)
        private const string CmdNext = "/next"; // przejdź do następnego kroku pipeline
        private const string CmdEnd = "/end";  // zakończ analizę tego meczu natychmiast
    ...
    Dictionary<string, string> filled)
        {
            ...
            {
                var token = m.Groups[1].Value; // np. FILL_11_1_JSON_BEZ_PREFIXU
                var baseToken = token;
                ...
                // jeśli krok chce *_JSON_BEZ_PREFIXU, to mapujemy na A_JSON/B_JSON (bo prompt 15 tego oczekuje)
                    ...
                    CancellationToken ct = default)
        {
            ...
            var steps = await _store.LoadAsync() ?? new List<StepDefinition>();
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                // Kursy z bukmachera – NAZYWAMY JAKO ODDS, nie P_imp (żeby model nie mieszał)
                if (step.KursBuch == true)
                {
                ...
                if (placeholders.Count > 0 && placeholders.All(p => filled.ContainsKey(p)))
                {
                ...
                //_ai.StartChat(systemPrompt);

                bool useWebSearch = step.WebSearch ?? enableWebSearch; // krok ma priorytet, null => global
                _ai.StartChat(systemPrompt,enableWebSearch: useWebSearch);
                ...
                // 2) jeśli GPT chce danych -> pytamy usera w UI
                ...
                // 3) parse KV i podmień placeholdery w doc
                int tries = 0;
                while (tries < 2)
                {
                    if (TryParseFillKv(assistant, placeholders, out var map, out var parseErr))
                    {
                        ...
                        // zapamiętaj wartości placeholderów (żeby nie musieć wysyłać całego doc w kolejnych krokach)
                        foreach (var kv in map)
                            filled[kv.Key] = kv.Value;


                        // 2) zapisz do match (<<FILL_X>> -> match.Surface / match.Round / ...)
                        ApplyFillMapToMatch(match, map);

                        // 3) jeżeli w doc lub kolejnych promptach używasz {Surface}/{Round}/{FormatMeczu},
                        //    to od razu je uzupełnij (bez psucia, jeśli jeszcze null)
                        doc = ReplaceTokens(doc, match);
        ...
        private static bool TryParseNeedUser(string text, out string question)
        {
            ...
            question = t.Substring("NEED_USER:".Length).Trim();
            return !string.IsNullOrWhiteSpace(question);
        }

        // Twoja dotychczasowa analiza (one-shot) – zostawiam, ale naprawiam token replacement (bez mutowania StepDefinition)
            ...
            CancellationToken ct = default)
        {
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                doc = ReplaceTokens(doc, match); // aktualizacja np {Surface}
                ...
                doc = await _ai.SendPromptAsync(stepPrompt, perStepCts.Token);
            }
        ...
        private static string ReplaceIfKnown(string text, string token, string? value)
        {
            if (string.IsNullOrWhiteSpace(value))
                return text; // zostaw {Surface} jeśli jeszcze nie znane
            return text.Replace(token, value);
        }

        /// <summary>
        /// Tryb CHAT: pipeline wysyła kroki do GPT, a potem pozwala użytkownikowi dyskutować w RichTextBox.
        /// Użytkownik wpisuje w rtb:
        ///  - /next => przejście do kolejnego kroku pipeline
        ///  - /end  => kończy analizę tego meczu natychmiast
        /// </summary>
        public async Task<string> AnalyzeAsyncChatGPT(
            MatchListItem match,
            Func<CancellationToken, Task<string>> waitUserMessageAsync,
            Action<string>? onChat = null,
            Action<int, int, string>? onStep = null,
            CancellationToken ct = default)
        {
            ...
            onChat?.Invoke($"=== START CHAT ANALIZY: {match.Tournament} | {match.PlayerA} vs {match.PlayerB} ({match.Day} {match.Hour}) ===");
            onChat?.Invoke($"Komendy: {CmdNext} (następny krok), {CmdEnd} (zakończ analizę meczu)");

            bool endMatchEarly = false;

            for (int i = 0; i < total; i++)
            {
                ct.ThrowIfCancellationRequested();
                if (endMatchEarly) break;
                ...
                onStep?.Invoke(i + 1, total, step.Title);
                onChat?.Invoke("");
                onChat?.Invoke($"--- KROK {i + 1}/{total}: {step.Title} ---");

                using var perStepCts = CancellationTokenSource.CreateLinkedTokenSource(ct);

                _ai.StartChat(stepPrompt);

                onChat?.Invoke("Ty: (wysyłam dokument do GPT...)");

                var firstUser = BuildInitialUserMessage(doc);
                var assistant = await _ai.SendChatAsync(firstUser, perStepCts.Token);

                onChat?.Invoke("GPT: " + assistant);
                doc = assistant;

                while (true)
                {
                    perStepCts.Token.ThrowIfCancellationRequested();

                    onChat?.Invoke("");
                    onChat?.Invoke($"Ty: (wpisz wiadomość, {CmdNext} aby kontynuować, {CmdEnd} aby zakończyć)");

                    var userMsgRaw = await waitUserMessageAsync(perStepCts.Token);
                    var userMsg = (userMsgRaw ?? "").Trim();

                    if (string.IsNullOrWhiteSpace(userMsg))
                        continue;

                    if (string.Equals(userMsg, CmdNext, StringComparison.OrdinalIgnoreCase))
                    {
                        onChat?.Invoke("Ty: /next");
                        break;
                    }

                    if (string.Equals(userMsg, CmdEnd, StringComparison.OrdinalIgnoreCase))
                    {
                        onChat?.Invoke("Ty: /end");
                        endMatchEarly = true;
                        break;
                    }

                    onChat?.Invoke("Ty: " + userMsg);
            ...
            onStep?.Invoke(total, total, "Gotowe");
            onChat?.Invoke("=== KONIEC CHAT ANALIZY MECZU ===");
        ...
        private static string ReplaceTokens(string text, MatchListItem m)
        {
            ...
            // nowe tokeny “semantyczne”
            result = ReplaceIfKnown(result, "{Surface}", m.Surface);
            //result = ReplaceIfKnown(result, "{Round}", m.Round);
