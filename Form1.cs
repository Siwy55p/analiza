...
namespace STSAnaliza
{
            ...
            try
            {
                for (int i = 0; i < _listMatches.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            // osobny krótszy timeout na rankingi (żeby nigdy nie wisiało 60 min)
                        ...
                        try
                        {
                            ...
                            srCts.CancelAfter(TimeSpan.FromSeconds(120)); // last52w = kilka/kilkanaście requestów
    ...
    new UnboundedChannelOptions
    {
        ...
        {
            //if (e.RowIndex < 0) return; // klik w nagłówek itp.
            ...
            //// Najprościej: indeks wiersza == indeks w _listMatches (jeśli grid jest zasilany tą listą w tej kolejności)
            ...
            //    // index1Based = e.RowIndex+1, total = _listMatches.Count (żeby log wyglądał podobnie jak w analizie listy)
